 ...
 */
export const OverlayApp: React.FC<OverlayAppProps> = ({ gameData, flags, alwaysVisible }) => {
  return (
    <>
      {alwaysVisible || gameData.state === "Loading" || gameData.state === "InGame" ? (
        <div className={"coh3stats-overlay"}>
          <div className={"coh3stats-overlay-left"}>
            {gameData.left.players.map((player, index) => (
              <PlayerEntry key={player.relicID + "_" + index} playerData={player} flags={flags} />
            ))}
          </div>
